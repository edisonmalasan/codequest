...
| Validation evidence | Chromium bundled dedicated scope fully passes (charter-backed targets correction, hard ack/fresh/source gates); unit 28/28 with clean lint/typechecks; Firefox post-update handshake failure root-caused to browser/environment limitation (frame-src-governed worker registration, uncontrolled page); cold-process S04 substitute 10/10 on headed installed Chrome (reverses opaque-era 0/10); WebKit offline cluster stable and environmental; opaque-era post-loop timeouts and 400% Check failures retained; physical/AT untested |
