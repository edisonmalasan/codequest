...
Status snapshot: 2026-09-24. Update this section when the phase or OpenSpec stage changes; planned work is not completed work.

| Item | Current status |
| --- | --- |
| Current phase | Phase 12 — Lesson Renderer implemented and verified; Sync and Archive have not started |
...
| Current OpenSpec change | `render-published-lessons` (Phase 12 Apply complete) |
| Current OpenSpec stage | Explore, Propose, and Apply complete; Sync and Archive have not started |
| Next execution step | Sync the completed Phase 12 deltas only when separately authorized, then archive the change |
| Next phase | Phase 12 — Lesson Renderer is active; Phase 13 has not started |
| Validation evidence | Phase 12 completed all 13 Apply tasks. Backend/frontend focused tests and Chromium lesson/Journey flows cover selected-snapshot assets, safe static Markdown, map navigation, keyboard hints, responsive reflow, image success/failure, and safe API/page errors; exact full-suite results are recorded in the active [tasks](../openspec/changes/render-published-lessons/tasks.md). |
...
| Current repository | Phase 0-12 foundations and implementation; production curriculum publication remains intentionally empty, the Phase 12 change is active pending Sync/Archive, and no Phase 13 editor/runtime behavior exists |
