...
| Item | Current status |
| --- | --- |
| Current phase | Phase 11 — Journey and Course UI proposal ready; implementation has not started |
...
| Current OpenSpec stage | Phase 11 Explore and Proposal complete; Proposal PR pending merge before Apply |
| Next execution step | Validate and merge the Phase 11 Proposal PR, then begin Apply on a separate implementation branch |
| Next phase | Phase 11 — Journey and Course UI is active; Phase 12 has not started |
| Validation evidence | Phase 10 completed all 17 Apply tasks and the required checks recorded in the [archived tasks](../openspec/changes/archive/2026-09-23-expose-versioned-curriculum-api/tasks.md); `openspec validate build-journey-course-map-ui --strict` passes for the Phase 11 proposal. |
...
| Current repository | Phase 0-10 foundations and canonical specifications; Phase 10 is implemented, synced, and archived with an intentionally empty production curriculum publication; Phase 11 contains planning artifacts only and no Phase 11 UI or later behavior exists |
