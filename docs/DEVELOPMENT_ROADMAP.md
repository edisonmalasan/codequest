...
Status snapshot: 2026-09-25. Update this section when the phase or OpenSpec stage changes; planned work is not completed work.

| Item | Current status |
| --- | --- |
| Current phase | Phase 14 — JavaScript Runtime Apply implemented and verified; Sync and Archive remain pending |
...
| Current OpenSpec change | `add-isolated-javascript-runtime` |
| Current OpenSpec stage | Phase 14 Apply complete on `feat/javascript-worker-runtime`; Sync and Archive have not started |
| Next execution step | Merge the Phase 14 Apply PR, sync canonical specs on a separate branch, then archive on a separate branch |
| Next phase | Phase 14 — JavaScript Runtime is active; Phase 15 has not started |
| Validation evidence | Phase 14 completed all 15 Apply tasks. The full suites pass with 207 frontend tests, 111 backend Vitest tests plus 3 backend history-script tests, 6 Chromium flows including isolated runtime/recovery/security probes, frontend and root lint/typecheck/build, API drift, strict OpenSpec validation, boundary/credential scans, and diff checks; exact evidence is recorded in the active [tasks](../openspec/changes/add-isolated-javascript-runtime/tasks.md). |
...
| Current repository | Phase 0-13 are complete and archived; Phase 14 Apply is implemented and awaiting Sync/Archive; production curriculum publication remains intentionally empty, and no Phase 15 preview behavior exists |
...
The `packages/api-client` line is a historical example, superseded by [approved decision C04](decisions.md) and [ADR 0002](adr/0002-api-contract-and-client-ownership.md). Generated client code belongs under `frontend/src/lib/api/generated/`; a root shared package requires at least two real independent consumers and a separately approved extraction. The archived [Phase 7 change](../openspec/changes/archive/2026-09-22-generate-typed-api-client/proposal.md) uses the frontend-local path, and the [api-contract-generation specification](../openspec/specs/api-contract-generation/spec.md) records the canonical contract. See [API contract generation](api-contract.md) for the regeneration workflow.
...
API contract generation was completed on 2026-09-22 through the [archived change](../openspec/changes/archive/2026-09-22-generate-typed-api-client/proposal.md). The canonical [api-contract-generation specification](../openspec/specs/api-contract-generation/spec.md) defines implemented backend routes as the sole contract source, frontend-local generated ownership, reproducible generation and drift enforcement, transport and security boundaries, and the exclusion of later domain behavior.

All 8 tasks passed backend, frontend, root, deterministic generation, positive and negative drift, boundary, credential, strict OpenSpec, CI, and diff verification. No authentication, authorization, curriculum publication, submissions, progress, rewards, offline sync, new product endpoints, or Phase 8 implementation was added.
...
The [archived Phase 8 change](../openspec/changes/archive/2026-09-22-establish-supabase-authentication/proposal.md) implements the Supabase session, backend verification, application-user bootstrap, ownership, permission, generated-client, security, and testing boundaries. The [authentication](../openspec/specs/authentication/spec.md) and [API-contract](../openspec/specs/api-contract-generation/spec.md) specifications are canonical. Production Google/GitHub provisioning, recovery, MFA, account linking/deletion, guest import, and Phase 9 curriculum behavior remain deferred.
...
Completion status (2026-09-23): implemented through an explicit reviewed `publication.yaml` selection, immutable fail-closed backend startup catalog, the five public read endpoints above, and the generated frontend client boundary. The production selection is intentionally empty, so the draft Q01 fixture is not exposed. The canonical specs are synced and the [completed change is archived](../openspec/changes/archive/2026-09-23-expose-versioned-curriculum-api/proposal.md). No database projection, Course entity, author mutation, learner execution, progress, XP award, unlock calculation, guest/offline sync, or Phase 11 UI was added.
...
Completion status (2026-09-24): implemented, verified, synced to the canonical [lesson-renderer](../openspec/specs/lesson-renderer/spec.md), [curriculum-api](../openspec/specs/curriculum-api/spec.md), and [journey-course-ui](../openspec/specs/journey-course-ui/spec.md) specifications, and [archived](../openspec/changes/archive/2026-09-24-render-published-lessons/proposal.md). `/quests/[slug]` renders the approved static Markdown subset, Quest context, graduated hints, safe selected-snapshot illustrations, responsive mobile reading, and explicit loading/failure states. Eligible/completed Course-map nodes link to published lessons while locked nodes remain inert. The backend asset operation is public, version-pinned, bounded to validated PNG/WebP files in the active snapshot, and returns safe not-found responses for invalid or unpublished requests. Production publication remains empty. No editor, learner execution, Check, validation, submission, progress acceptance, XP award, authentication/database change, or Phase 13+ behavior was added.
...
Completion status (2026-09-25): implemented, verified, synced to the canonical [`editor-workspace`](../openspec/specs/editor-workspace/spec.md) specification, and [archived](../openspec/changes/archive/2026-09-25-build-reusable-editor-workspace/proposal.md). The reusable frontend workspace composes the planned editor, toolbar, file tabs, console, test-results, runtime-status, and action surfaces from parent-supplied owner/workspace/file inputs. CodeMirror provides JavaScript highlighting, line numbers, indentation, bounded autocomplete, accessible labeling, and controlled source updates. Owner/workspace/file-scoped IndexedDB drafts support debounced autosave, explicit save, failure feedback, restoration, and confirmed selected-file reset. The development-only `/editor-workspace` route and automated browser flow verify keyboard interaction and narrow-width reflow. Runtime-facing panels remain truthful presentation surfaces: no learner execution, validation, submission, progress, XP, unlock, curriculum integration, backend behavior, or Phase 14 work was added.
