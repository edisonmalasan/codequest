...
Status snapshot: 2026-09-23. Update this section when the phase or OpenSpec stage changes; planned work is not completed work.

| Item | Current status |
| --- | --- |
| Current phase | Phase 11 — Journey and Course UI Apply complete and verified; Sync and Archive have not started |
...
| Current OpenSpec change | `build-journey-course-map-ui` — [proposal](../openspec/changes/build-journey-course-map-ui/proposal.md) |
| Current OpenSpec stage | Phase 11 Apply complete and verified on its implementation branch; Apply PR pending merge before Sync |
| Next execution step | Merge the Phase 11 Apply PR, then sync `journey-course-ui` on a separate spec-sync branch |
| Next phase | Phase 11 — Journey and Course UI is active; Phase 12 has not started |
| Validation evidence | Phase 11 completed all 12 Apply tasks. Frontend and root lint/typecheck/test/build, the 4-test Playwright suite, API drift, strict OpenSpec validation, and `git diff --check` pass; Sync and Archive remain pending. |
...
| Current repository | Phase 0-10 foundations and canonical specifications plus the verified Phase 11 Journey/Course-map implementation awaiting its Apply merge; production curriculum publication remains intentionally empty and no Phase 12 behavior exists |
...
Apply status (2026-09-23): implemented and verified on the Phase 11 Apply branch. The canonical `/journeys/[slug]` page consumes the generated public curriculum client, builds a prerequisite-aware display model, and presents Journey context, overall progress, ordered chapters, and completed/current/available/locked quest states with explicit loading, empty, failure, accessibility, reduced-motion, and responsive behavior. Because no approved progress source exists yet, the production route supplies an empty completion snapshot and states that saved progress is not loaded; accepted progress and unlock authority remain backend-owned. No curriculum publication, backend/OpenAPI/generated-client change, lesson rendering, editor/runtime, or Phase 12+ behavior was added. Sync and Archive remain pending.
