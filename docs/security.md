...
| Unsafe lesson/MDX/custom tests | Trusted repository authoring/review, restricted static Markdown rendering, selected-snapshot local images, and data-only test context; no arbitrary learner execution in NestJS | Phase 9 authoring allowlist plus Phase 12 renderer, asset-path, inert-markup, and browser fixture tests; no raw HTML/MDX execution |
