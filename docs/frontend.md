...
Console, test-results, and runtime-status surfaces are presentation-only in Phase 13. Their defaults state that execution and checks are unavailable, and parent-supplied display text is rendered without running source or inferring correctness. No Run, Check, Submit, completion, or reward action exists. The Phase 14 runtime can later populate these surfaces through a separately approved adapter without moving execution into the workspace shell.
