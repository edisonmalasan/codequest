...
## F01 Phase 1 selection - 2026-09-18

Delegated review (not an independent audit): the **dedicated candidate is selected as the Phase 1 recommended mechanism** — fresh literal-URL HTTP Worker per run under `worker-src 'none'`, exact-origin private cleanup with 100ms fallback, generation-bound bounded output, and fixed script-disabled supplied-shell presentation with text equivalent. Basis: installed-Chrome 63/63, Chromium bundled scope, Firefox partitioned component coverage (post-harness-allowlist repair, load contention proven environmental), WebKit except the reviewed S06 offline-emulation cluster, empty sink records in every authority probe, P11 holding in all measured runs. Historical executable-HTML no-go (AP02) retained. Production adoption requires a Phase 2+ proposal. F02 validated scope is desktop-first with physical/AT obligations deferred pre-beta. Full record: [F01/F02 selections](../decisions.md#f01f02-phase-1-selections---2026-09-18), [AP03](../decisions.md#ap03-delegated-s01-s06-scope-approval---2026-09-18).
