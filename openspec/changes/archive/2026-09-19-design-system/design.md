...
- **Native elements + ARIA instead of Base UI/Radix**: native `<button>`, `<input>`, `<select>` carry built-in keyboard/screen-reader behavior; custom Tabs/Tooltip/Toast/Dropdown/Drawer/Dialog implement roving tabindex and focus placement, focus trap (Dialog/Drawer), `aria-live` (Toast), and outside-interaction dismissal with unit tests. Base UI adoption deferred until product screens prove the need — installing a primitive library for 13 components now adds API surface without a consumer.
- **Stack conformance note (shadcn/ui + Base UI)**: AGENTS.md names shadcn/ui + Base UI as the frontend direction, and this change deliberately ships native primitives first. This is conformance by staging, not divergence: the Phase 4 contract (presentational components, token API, tested keyboard/a11y behavior) is fully satisfied natively, component prop APIs are stable (`variant`/`size` via `cva`, display-only game props), and the first product-screen phase that needs unmet primitive behavior (e.g. composable menu/popover primitives) adopts Base UI behind these same APIs without breaking consumers. No migration is owed by Phase 4 itself.
