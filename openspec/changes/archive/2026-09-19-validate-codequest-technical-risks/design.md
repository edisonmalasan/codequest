...
Public-cache repair boundary: a poisoned or deleted public cache must not make every subsequent valid run unavailable. The installed trusted runner service worker also embeds the same pinned public resource bytes and headers. It can reconstruct a rejected/missing entry from those immutable bytes after hash/CSP verification; it never serves learner cache bytes or uses network fallback. This maintains cold-offline trusted recovery without reusing a learner Worker. Cache poisoning tests require a successful <=1s fresh run with expected output and exact source, not acceptance of the malicious response. Without verified immutable backup, responses fail closed with 503.

Dedicated startup isolation: the execution bootstrap no longer registers or waits for the runner service worker. Public preparation owns registration; online first execution can use the literal HTTP Worker with unchanged response CSP, and prepared offline navigation uses the existing installed worker/cache. Execution bootstrap loading and the private handshake remain inside the unchanged deadline. The lifecycle collector records all ten loops (including unwitnessed startups) and all 100 cycles before assertions, with a bounded 1900ms witness observation and a test-harness allowance covering every original per-run deadline. This changes collection completeness only: no runtime limit or freshness criterion is raised, and a missing witness or budget miss still fails.

Frozen public-response guard: all cached bodies are read through a bounded stream with a 64KiB trusted-public-resource ceiling before SHA-256 verification; hostile body/header mutations cannot trigger an unbounded JS body buffer. Browser-native chunk allocation is still not a hard memory quota. Delivered responses use only the pinned manifest's Content-Type/CSP headers, so injected reporting/refresh/cookie metadata never becomes browser authority. Record online metadata/reporting-header poisoning, bounded 1MiB cache-body poisoning, zero sink requests, immutable-byte repair and <=1s fresh recovery. Learner source/output/message limits and all B01-B09 criteria are unchanged.
