...
The system SHALL derive its frontend API types from the backend's OpenAPI 3 document for implemented routes. The generated contract SHALL retain versioned paths, HTTP methods, request and response schemas, status codes, and the existing safe error envelope where those responses are implemented. It SHALL NOT describe planned but unimplemented domain operations as callable endpoints.

#### Scenario: Current contract is generated

- **WHEN** the contract is generated from the current backend application
- **THEN** it includes the implemented `GET /api/v1/health` operation with its success response and no invented curriculum, account, learning, or gamification operations
