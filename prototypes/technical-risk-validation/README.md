...
`node scripts/native-chrome-diagnostics.mjs --retention-control` adds ten/thirty-second natural-idle resource snapshots after the same workload; it does not force GC or extend recovery/readiness budgets. Current-process CPU sums may decrease when processes exit and are not cumulative workload CPU. Run `test:browser tests/nested-worker.spec.ts` for permissive/restricted nested Blob request controls and fixed child-first-message loop rejection/recovery, recording unavailable constructor/bootstrap paths separately. The persistent-bootstrap experiment was explicitly authorized on 2026-09-14 and is implemented in `src/opaque-compartment.ts` and `public/opaque-bootstrap.js`; it is not a production selection or a reviewed Phase 1 pass.


### Authorized persistent opaque bootstrap experiment

Each BrowserRuntime retains at most one trusted opaque bootstrap and creates a fresh learner Worker per run. A one-time actual-window handshake transfers a private host/bootstrap MessagePort, never transferred to learner code. Run/task/version-correlated stop acknowledgments follow calling Worker.terminate; a missing private acknowledgment removes the compartment after the predeclared 100ms fallback. Execution retains its 2s deadline plus 1s recovery, with fresh runs <=1s. Bootstrap loading is measured, not prewarmed. Learner output remains bounded untrusted window data.

Idle trusted bootstrap frames are recorded separately from active Worker/presentation frames. `window.__risk.dispose()` explicitly closes/removes all prototype runtime compartments, with Blob revocation before acknowledged disposal. Private acknowledgment measures host/bootstrap cleanup of Worker handles; it is not proof of synchronous descendant-thread termination or a hard resource quota.

Run `corepack pnpm@10.30.3 exec playwright test tests/bootstrap-lifecycle.spec.ts` for reuse, forged output/control, deliberate missing-ack fallback, supersession and Chrome-specific actual Worker-target observations. Non-Chromium engines explicitly skip CDP target instrumentation. `node scripts/worker-target-diagnostics.mjs` records bounded target/current-process CPU observations without manual debugger attachment; `--attach-control` records a separate debugger-attachment comparison, and `--chrome` selects actual installed headed Chrome. Diagnostic target retention must not be inferred to mean either live execution or successful cleanup without additional evidence. Historical failures remain retained.


`node scripts/native-startup-diagnostics.mjs` records document-only constructor/control/result timestamps for 100 native fresh runs. `--hostile-first` precedes them with ten witnessed-or-explicitly-unwitnessed loop trials; `--dedicated` compares the existing dedicated candidate. Test-only instrumentation does not change learner source, CSP or acceptance budgets, and its passing subsets never erase uninstrumented failures. Source/script/build hashes and bounded events are retained. `src/bootstrap.test.ts` executes the real trusted bootstrap against controlled Worker/port fixtures to verify termination before terminal delivery and marker/private-ack ordering; it is not a browser CPU-containment test.
