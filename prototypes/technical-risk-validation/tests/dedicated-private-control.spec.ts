...
test('E02/E03 private cleanup remains separate from forged learner output and bootstrap reuse', async ({ page, browser }, info) => {
  ...
  const first = await page.evaluate(() => window.__risk.run('console.log("first")', 'dedicated'));
  ...
  const forged = await page.evaluate(() => window.__risk.run('self.postMessage(JSON.stringify({type:"cleaned",ticket:"FORGED",activeWorkers:0}));while(true){}', 'dedicated'));
...
test('E02 missing private acknowledgment removes bootstrap within bounded fallback and restarts', async ({ page, browser }, info) => {
  ...
  const fresh = await page.evaluate(() => window.__risk.run('console.log("after fallback")', 'dedicated'));
  expect(fresh.status).toBe('success');
  expect(fresh.elapsed).toBeLessThan(1000);
  await page.evaluate(() => window.__risk.dispose());
  await expect(page.locator('iframe')).toHaveCount(0);
...
test('E02 superseded runs and unsolicited stop-shaped window messages cannot replace fresh output', async ({ page, browser }, info) => {
  const result = await page.evaluate(async () => {
    const old = window.__risk.run('await new Promise(r=>setTimeout(r,500));console.log("stale")', 'dedicated');
    const fresh = window.__risk.run('console.log("current")', 'dedicated');
...
test('E02 replayed actual prior output from the retained bootstrap cannot fail the current run', async ({ page, browser }, info) => {
  const captured = await page.evaluate(async () => {
    ...
    try { const first = await window.__risk.run('console.log("prior")', 'dedicated'); return { first, packet }; }
  ...
  await page.evaluate(() => { window.__bootstrapLoop = window.__risk.run('await new Promise(r=>setTimeout(r,300));console.log("current")', 'dedicated'); });
