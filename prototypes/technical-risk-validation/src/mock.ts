...
export function snapshotFrom(raw: unknown): Snapshot | null {
  ...
  if (raw.event.length > 100 || raw.owner.length > 100 || raw.source.length > 65536) return null;
...
export class SimulationLedger {
  ...
  accept(principal: string, snapshot: Snapshot): Receipt {
    ...
    if (snapshot.quest !== 'Q01' || (snapshot.assessmentVersion !== '1' && snapshot.assessmentVersion !== 'editorial-1')) return { status: 'retry-required', simulatedReward: false, message: 'Unsupported or retired version; retry without losing draft' };
