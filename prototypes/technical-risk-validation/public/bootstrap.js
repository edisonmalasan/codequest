const bootstrapId = location.hash.slice(1);
// Trusted dedicated document only. The private port never enters a learner Worker.
let control;
let current;
let identity;
let publicWorkerUrl;
  ...
  control.onmessage = event => {
    ...
    if (command.type === 'start' && !current) {
      ...
      if (command.dispose === true && publicWorkerUrl) {
        // HTTP public Worker URL has no Blob resource to revoke.
